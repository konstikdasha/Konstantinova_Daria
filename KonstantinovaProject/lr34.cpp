...
void lr34::SaveToFile(ofstream& fout)
{
	fout << pGroup.size() << '\n'
		<< csGroup.size() << '\n'
		<< Pipe::pMaxId << '\n'
...
ostream& operator << (ostream& out, const lr34& lr)
{
	out << "Количество труб - " << lr.pGroup.size() << "\n";
	for (const pair<int, Pipe>& p : lr.pGroup)
	{
		out << "Труба " << p.first << ".\n";
		out << p.second;
	}
	out << "Количество КС - " << lr.csGroup.size() << "\n";
	for (const pair<int, CompressorStation>& cs : lr.csGroup)
	{
...
vector<vector<int>> lr34::GetEdgesAndVertexes(unordered_map<int, int>& indexVertexes, int& n)
{
	set<int> vertexes;
...
bool lr34::HasPipe() const
{
	return pGroup.size() != 0;
}

bool lr34::HasCs(int count) const
{
...
bool lr34::CanBeUsed(const Pipe& p) const
{
	return p.inCsId > 0 &&
		p.outCsId > 0 &&
		!p.GetRepairing() &&
